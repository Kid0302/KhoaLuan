...
namespace BLL
{
	public class BLL_NhanVien
	{
        ...
        public bool CheckDangNhap(string tk, string mk)
        {
            var check = qlst.NhanViens.Any(nv => (nv.SDT == tk || nv.Email == tk) && nv.MatKhau == mk && nv.TrangThaiXoa == 1 && nv.MaNV != 1);
            return check;
        }
        ...
        public async Task<System.Drawing.Image> GetAnhNhanVienAsync(string tk, string mk, Size newSize)
        {
            ...
            if (nhanVien != null)
            {
                ...
                try
                {
                    ...
                    {
                        await storage.DownloadObjectAsync("qlsieuthi-54c8e.appspot.com", firebasePath, imageStream);
        ...
        public List<ListFrmNV> Get_ListNV()
        {
            var listnv = (
                ...
                where nv.TrangThaiXoa == 1 && nv.MaNV != 1
        ...
        {
            return qlst.NhanViens.FirstOrDefault(nv => nv.MaNV == maNV && nv.TrangThaiXoa == 1 && nv.MaNV != 1);
        }
        ...
        public List<ListFrmNV> Search_ListNV(string tenNV)
        {
            var searchList = (
                from nv in qlst.NhanViens
                join cv in qlst.ChucVus on nv.MaCV equals cv.MaCV
                where nv.TenNV.Contains(tenNV) && nv.TrangThaiXoa == 1 && nv.MaNV != 1
                select new ListFrmNV
                {
                    MaNV = nv.MaNV,
                    TenNV = nv.TenNV,
        ...
        public List<ListFrmNV> Search_ListNV_MaNV(int maNV)
        {
            var searchList = (
                from nv in qlst.NhanViens
                join cv in qlst.ChucVus on nv.MaCV equals cv.MaCV
                where nv.MaNV == maNV && nv.TrangThaiXoa == 1 && nv.MaNV != 1
        ...
        public List<ListFrmNV> Search_ListNV_GTinh(string gtinh)
        {
            var searchList = (
                from nv in qlst.NhanViens
                join cv in qlst.ChucVus on nv.MaCV equals cv.MaCV
                where nv.GioiTinh.Contains(gtinh) && nv.TrangThaiXoa == 1 && nv.MaNV != 1
                select new ListFrmNV
                {
                    MaNV = nv.MaNV,
                    TenNV = nv.TenNV,
        ...
        private async Task DeleteOldImageFromFirebase(int maNV, string oldImageName)
        {
            try
            {
                ...
                {
                    await storage.DeleteObjectAsync("qlsieuthi-54c8e.appspot.com", remoteImagePath);
        ...
        public List<ListFrmNV_IsDelete> GetListNV_IsDeleted()
        {
            var listkh = (
                from nv in qlst.NhanViens
                where nv.TrangThaiXoa == 0 && nv.MaNV != 1
        ...
        public List<ListTopNV> Top10NhanVienDoanhThuTrongThang()
        {
            ...
            var query = from nv in qlst.NhanViens
                        ...
                        where hd.NgayLap.Month == currentMonth && hd.NgayLap.Year == currentYear && nv.TrangThaiXoa == 1 && nv.MaNV != 1
